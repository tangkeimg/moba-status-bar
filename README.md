...
Network monitoring is intentionally compact and lightweight. It stays hidden by default, samples the most relevant active network interface when enabled, keeps upload speed hidden by default so the status bar stays narrow, and formats the rate display to stay visually stable as values change.
